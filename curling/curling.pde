...
final int   SIMS_PER_EXPERT_SHOT  = 2;  // repeated simulations per expert shot
final int   SIMS_PER_NEXT_EXPERT  = 2;  // repeated simulations per next-shot expert
...
void onLoadModelSelected(File selection) {
    if (selection == null) return;
    modelStorage.loadActiveModel(selection.getAbsolutePath());
    trainingStatus = modelStorage.lastMessage;
}
...
void runAiTestSim() {
    ...
    aiTestLastShot = gradientEnsemble.bestShot(aiTestStones, 1, TEAM_RED, 0,
                                            shotSelector, true, false);
...
// Alternative to neural network, simulates N shots and picks the best performing one.
...
void maybeAiShoot() {
    ...
    if (activeAiType == 1) {
        ...
        shot = gradientEnsemble.bestShot(game.stones, game.stonesRemaining(TEAM_YELLOW),
                                        lastTeam, remainingShotsAfterCurrent,
                                        shotSelector, true, false);
    }
    game.fire(shot);
}
...
void drawShotPreview(Shot shot, color teamColor) {
    ...
    beginShape();
    for (PVector p : path) {
        PVector s = sheet.worldToScreen(p);
        vertex(s.x, s.y);
    }
    endShape();
    popStyle();
